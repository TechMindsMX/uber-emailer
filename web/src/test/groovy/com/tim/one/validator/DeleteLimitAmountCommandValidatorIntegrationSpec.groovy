...
@Transactional
public class DeleteLimitAmountCommandValidatorIntegrationSpec extends Specification {

	@Autowired
	CommandValidator validator

	@Unroll
	def """When we have a user with #userId as id, and user with #destinationId as id and want to delete maximum amount 
			to transfer, when we validate we expecting that result is: #result"""() {
		given: "An cash out command"
		def command = new DeleteLimitAmountCommand()
